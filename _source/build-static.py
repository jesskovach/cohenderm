...
def jpeg_size(path):
    """Width/height from a JPEG's SOF marker. No dependencies."""
...
def finish_html(html, is_first_page_image_eager=True):
    """Post-process a finished page:
       - resolve the JS-injected portal links so crawlers and no-JS visitors
         get a real destination instead of href="#"
       - stamp intrinsic width/height on every image (prevents layout shift,
         a Core Web Vitals factor) and lazy-load everything below the fold
    """
    def resolve_portal(m):
        tag = m.group(0)
        if 'href="#"' not in tag:
            return tag
        if any(k in tag for k in ("portal-signup",)):
            return tag.replace('href="#"', f'href="{PORTAL_SIGNUP_URL}"')
        if any(k in tag for k in ("portal-link", 'id="portalHeader"', 'id="portalNav"')):
    ...
    def stamp(m):
        ...
        if not src or "width=" in tag:
            return tag
        dims = jpeg_size(OUT / src.group(1).lstrip("/"))
        if not dims:
            return tag
        w, h = dims
        extra = f' width="{w}" height="{h}"'
        if seen["first"] and is_first_page_image_eager:
            extra += ' fetchpriority="high"'
            seen["first"] = False
        else:
            extra += ' loading="lazy" decoding="async"'
        return tag[:-1].rstrip("/") + extra + ">"
    return re.sub(r'<img\b[^>]*>', stamp, html)

def canon(route):
    """Canonical URL for a route. The host serves /about/ and 307s /about,
    so every canonical link and sitemap entry uses the slashed form."""
    return ORIGIN + ("/" if route == "/" else route.rstrip("/") + "/")
...
js = js.replace(' <a href=\\"#/account\\">Preview the patient account (demo) \u2192</a>', '')
...
def drop_account_links(t):
    return re.sub(r'\s*<a [^>]*href="#/account"[^>]*>.*?</a>', '', t)

def rewrite_links(t):
    t = drop_account_links(t)
    # any same-site anchor: #/#book, #/#below → /#book, /#below
    t = re.sub(r'href="#/#([a-zA-Z0-9_-]+)"', r'href="/#\1"', t)
  ...
  "@graph": [
    {{
      ...
      "sameAs": [
        ...
        "https://www.instagram.com/skin_with_rachel_cohen_np/"
      ]
    }},
    {{
      "@type": "WebSite",
...
print(f"also: sitemap.xml, robots.txt")
